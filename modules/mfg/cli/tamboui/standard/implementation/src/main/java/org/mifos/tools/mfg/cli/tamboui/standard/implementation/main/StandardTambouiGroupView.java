...
@Component
final class StandardTambouiGroupView {
    private final StandardTambouiGroupController controller;
    ...
    Panel render() {
        ...
        return panel(" Groups: %s ".formatted(controller.size()), listElement)
                .vertical()
                .rounded()
                .borderColor(DARK_GRAY)
                .focusedBorderColor(CYAN)
                .id(MIFOS_TOOLS_MFG_CLI_TAMBOUI_VIEW_GROUP)
                .focusable();
    }
    ...
    private void buildList() {
        ...
        for (var item : controller.items()) {
            listElement.add(text(Optional.ofNullable(item.getDescription()).orElse(item.getName())));
        }
